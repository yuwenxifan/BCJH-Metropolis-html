...
function __asyncjs__fetch_userData() {
  // let id = document.getElementById("userId").value;
  let id = 998;
  return Asyncify.handleAsync(
    async () => {
      printLog("waiting for a fetch");
      let response = await fetch("https://bcjh.xyz/api/download_data?id=" + id);
      // if (!id) {
      ...
      //   }
      // } else {
      //   response = await fetch("https://bcjh.xyz/api/download_data?id=" + id);
...
// end include: preamble.js
  ...
  var TTY = {ttys:[],init:function () {
        ...
        FS.registerDevice(dev, TTY.stream_ops);
          ...
          if (val === null || val === 10) {
            printLog(UTF8ArrayToString(tty.output, 0));
            tty.output = [];
          } else {
  ...
  var FS = {root:null,mounts:[],devices:{},streams:[],nextInode:1,nameTable:null,currentPath:"/",initialized:false,ignorePermissions:true,ErrnoError:null,genericErrors:{},filesystems:null,syncFSRequests:0,lookupPath:(path, opts = {}) => {
        ...
        LazyUint8Array.prototype.cacheLength = function LazyUint8Array_cacheLength() {
          ...
          if (usesGzip || !datalength) {
            ...
            printLog("LazyFiles on gzip forces download of the whole file when length is accessed");
          }
